...
class GomokuGame:
    ...
    def _notify_agents_game_over(self):
        if self.game_over_notified:
            return

        self.game_over_notified = True
        winner_symbol = self.winner.agent_symbol if self.winner else None
        for agent in (self.p1, self.p2):
            if hasattr(agent, "on_game_over"):
                agent.on_game_over(self.board, winner_symbol)
    ...
    def play_turn(self):
        ...
        if self.winner or np.count_nonzero(self.board) == BOARD_SIZE * BOARD_SIZE:
            self._notify_agents_game_over()
